...
def train():
  ...
  val_duration_losses = Deque(maxlen=100)
  ...
  for step in tr:
    ...
    if step % 10 == 0:
      ...
      val_loss, val_duration_loss, val_aux, predicted_mel, gt_mel = val_loss_fn(params, aux, rng, val_batch)
      val_losses.append(val_loss)
      val_duration_losses.append(val_duration_loss)
    ...
    if step % 1000 == 0:
      ...
      val_duration_loss = sum(val_duration_losses).item() / len(val_duration_losses)
      tr.write(f'step {step}  train loss {loss:.3f}  val loss {val_loss:.3f} val dur loss {val_duration_loss:.3f}')
